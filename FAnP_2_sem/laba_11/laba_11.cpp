...
int count(Tree* t);  //Подсчет количества правых листьев
int countL(Tree* t);  //Подсчет количества левых листье 
int countALL(Tree* t);  //Подсчет количества листье
int var2(Tree* t, int level);  //Подсчет количества слов
bool delAll(Tree* t);             //Очистка дерева

int c = 0;  int kR = 0;  int kL = 0;     //количество слов
...
void main()
{
	...
	for (;;)
	{
		...
		cout << "5 -	вывод дерева\n";
		cout << "6 -	подсчет количества правых веток ( вар 15)\n";
		cout << "66 -	вывод правых листьев ( вар 2)\n";
		cout << "666 -	кол-во левых листьев ( вар 5)\n";
		cout << "6666 -	кол-во всех листьев ( вар 7)\n";
		...
		case 6:
			n = count(Root);
			cout << " Листье в правой ветке ( без корня) = " << n << endl; break;
		case 666:
			n = countL(Root);
			cout << " Листьев в левой ветке ( без корня) = " << n << endl; break;
		case 6666:
			n = countALL(Root);
			cout << " Листьев всего =  " << n << endl; break;
		case 66:

			if (Root->key >= 0)
			{
				cout << "Правые листья дерева повернутые на 90 град. влево" << endl;
				var2(Root, 0);
			}
			else cout << "Дерево пустое\n"; break;
...
int countL(Tree* t) //var5 Подсчет количества слов
{
...
int countALL(Tree* t) //va7 Подсчет количества листьев дерева 
{
